...
## 2026-03-10 (midi exec mode extension: before-external)

### Goal
- Add a third per-slot MIDI exec mode without regressing existing behavior:
  - `after` (legacy)
  - `before` (existing cable-0 replacement path)
  - `before-external` (new cable-2 reroute path)

### Change implemented
- Extended `midi_exec` parsing/serialization and persistence to support mode `2` / `"before-external"` across:
  - chain host patch state (`patch:midi_exec`)
  - shadow chain mgmt + slot param set/get
  - set-page config save/load
  - shadow state save/load
  - shadow UI control range + labels/export.
- Kept existing before-mode pad-only interception unchanged:
  - only grid pad notes (`68..99`) are intercepted/blocked from raw cable 0.
  - non-pad MIDI remains on cable 0 unchanged.
- Added slot-aware cable selection in shim queue drain for internal-only before modes:
  - `before` -> reinject on cable `0` (replace-in-place)
  - `before-external` -> reinject on cable `2` (reroute external path)

### Verification
- `bash tests/host/test_chain_midi_exec_before.sh` PASS
- `bash tests/shadow/test_shadow_midi_exec_before_wiring.sh` PASS
- `bash tests/shadow/test_midi_to_move_injection_stability.sh` PASS
- `bash tests/host/test_chain_v2_midi_source_gate.sh` PASS
- `bash tests/host/test_chain_midi_fx_idle_activity_window.sh` PASS
- Full build/package: `/bin/zsh -lc 'PATH=/opt/homebrew/bin:$PATH ./scripts/build.sh'` PASS
